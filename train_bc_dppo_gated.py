...
from models.dppo_cnn_gated import GatedCNNDPPOPolicy
from models.dppo_mae import observation_to_eval_frame, save_gif
from models.pretrain_models import VTT, VTMAE

DPPOPolicy = GatedCNNDPPOPolicy
...
def load_policy_checkpoint(policy: DPPOPolicy, checkpoint_path: str, strict: bool = False) -> None:
    ...
    print(
        ...
        f"(loaded={len(state_dict)}, skipped_shape_or_missing={skipped}, "
...
def evaluate_offline(
    ...
    try:
        for batch_idx, (observations, actions) in enumerate(progress):
            ...
            gate = getattr(policy.encoder, "last_gate", None)
            if gate is not None:
                total_gate += float(gate.mean().item()) * batch_size
                total_gate_examples += batch_size
    ...
    return {
        ...
        "gate_mean": total_gate / total_gate_examples if total_gate_examples > 0 else 0.0,
    }
...
def evaluate_in_env(
    ...
    if gif_frames is not None and gif_path:
        if save_gif(gif_frames, gif_path, fps=gif_fps):
            print(f"[BC] eval GIF saved to {gif_path}")
...
def set_policy_gate_min(policy: DPPOPolicy, value: float) -> None:
    if hasattr(policy, "set_gate_min"):
        policy.set_gate_min(value)


def get_policy_gate_min(policy: DPPOPolicy) -> float:
    if hasattr(policy, "get_gate_min"):
        return float(policy.get_gate_min())
    return 0.0


def gate_min_for_epoch(epoch: int, config) -> float:
    start = float(config.gate_min_start)
    end = float(config.gate_min_end)
    anneal_epochs = int(config.gate_min_anneal_epochs)
    if anneal_epochs <= 0 or abs(start - end) <= 1e-12:
        return start

    progress = min(max((int(epoch) - 1) / float(anneal_epochs), 0.0), 1.0)
    if config.gate_min_schedule == "linear":
        weight = 1.0 - progress
    else:
...
def update_ema_policy(ema_policy: DPPOPolicy, policy: DPPOPolicy, decay: float) -> None:
    ...
    for key, value in policy_state.items():
        ...
        if key.endswith("gate_min_value"):
...
def parse_args():
    parser = argparse.ArgumentParser("Behavior cloning pretraining for GatedCNNDPPOPolicy")
    ...
    parser.add_argument("--run_name", type=str, default="bc_dppo_gated_pretrain")
    ...
    parser.add_argument("--experiment_name", type=str, default="bc_dppo_gated_pretrain")
    ...
    parser.add_argument("--gate_min_start", type=float, default=0.0)
    parser.add_argument("--gate_min_end", type=float, default=0.0)
    parser.add_argument("--gate_min_anneal_epochs", type=int, default=0)
    parser.add_argument("--gate_min_schedule", type=str, default="cosine", choices=["cosine", "linear"])
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="./logs/bc_gated_checkpoints")
    parser.add_argument("--best_val_path", type=str, default="./logs/bc_gated_checkpoints/best_val.pt")
    parser.add_argument("--best_eval_path", type=str, default="./logs/bc_gated_checkpoints/best_eval.pt")
    parser.add_argument("--final_model_path", type=str, default="./logs/bc_gated_policy_final.pt")
    parser.add_argument("--log_path", type=str, default="./logs/bc_gated_metrics.jsonl")
    ...
    if args.gate_min_start < 0.0 or args.gate_min_start > 1.0:
        parser.error("--gate_min_start must be in [0, 1]")
    if args.gate_min_end < 0.0 or args.gate_min_end > 1.0:
        parser.error("--gate_min_end must be in [0, 1]")
    if args.gate_min_anneal_epochs < 0:
        parser.error("--gate_min_anneal_epochs must be non-negative")
...
def main():
    config = parse_args()
    if config.experiment_name == "bc_dppo_gated_pretrain" and config.run_name:
    ...
    print(
        f"[BC] gate_min start={config.gate_min_start} end={config.gate_min_end} "
        f"anneal_epochs={config.gate_min_anneal_epochs} schedule={config.gate_min_schedule}"
    )
    ...
    if needs_offline_data:
        ...
        print("[BC] eval-only env-only mode: skipping expert dataset discovery")

    if config.pretrained_mae_path:
        print("[BC] pretrained_mae_path is ignored by gated CNN policy")
    ...
    policy = DPPOPolicy(
        ...
        gate_min_value=config.gate_min_start,
    ...
    if config.resume_path:
        ...
        if isinstance(checkpoint, dict) and "optimizer_state_dict" in checkpoint:
            try:
                ...
                print(f"[BC] skipped optimizer state from resume checkpoint: {exc}")
        if ema_policy is not None:
    ...
    print(f"[BC] current gate_min={get_policy_gate_min(policy):.6f}")
    ...
    if config.eval_only:
        ...
        log_record = {
            ...
            "eval/gate_min": get_policy_gate_min(eval_policy),
        }
        if val_loader is not None and config.eval_every_epochs > 0:
            ...
            if offline_metrics is not None:
                log_record.update({
                    ...
                    "val/gate_mean": offline_metrics["gate_mean"],
                })
        if eval_env is not None:
        ...
        if "val/action_mse" in log_record:
            ...
            summary_parts.append(f"val_gate={log_record['val/gate_mean']:.4f}")
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        current_gate_min = gate_min_for_epoch(epoch, config)
        set_policy_gate_min(policy, current_gate_min)
        if ema_policy is not None:
            set_policy_gate_min(ema_policy, current_gate_min)
        if config.freeze_encoder:
            policy.encoder.eval()
        ...
        log_record = {
            ...
            "train/gate_mean": train_metrics["gate_mean"],
            "train/gate_min": current_gate_min,
        ...
        summary_parts = [
            ...
            f"gate={train_metrics['gate_mean']:.4f}",
            f"gate_min={current_gate_min:.4f}",
        ]
        if "val/action_mse" in log_record:
            summary_parts.append(f"val_mse={log_record['val/action_mse']:.6f}")
            summary_parts.append(f"val_gate={log_record['val/gate_mean']:.4f}")
